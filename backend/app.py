...
def new_run():
    ...
    # Condition & model are from phaser
    condition = str(data.get("condition", "neutral")).strip()
    model = str(data.get("model", "default")).strip()
    ...
    Expected JSON body:
            {
                ...
                "provider": "openai", // from phaser
                "model": "gpt-4o-mini"  // from phaser
            }
